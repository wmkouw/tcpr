function exp_ssb_credit(varargin)
% Sample selection bias experiments on the credit dataset
...
try
    ...
    cd('../data/credit')
    [D,y] = get_credit('save', true);
    copyfile credit.mat ../../experiment-credit/
    cd('../../experiment-credit')
end 
...
switch p.Results.clf
    ...
    case 'tcp-ls' 
        exp_ssb_tcp(D,y, 'nN', p.Results.nN, 'nM', p.Results.nM, 'setDiff', p.Results.setDiff, 'nR', p.Results.nR, 'nF', p.Results.nF, 'maxIter', p.Results.maxIter, 'xTol', p.Results.xTol, 'saveName', [p.Results.saveName p.Results.dataName '_prep' p.Results.prep{logical(cellfun(@isstr, p.Results.prep))} '_nR' num2str(p.Results.nR) '_'], 'alpha', p.Results.alpha, 'lambda', p.Results.lambda, 'clf', 'tcp-ls', 'lr', p.Results.lr);
    case 'kmm-lsq'
