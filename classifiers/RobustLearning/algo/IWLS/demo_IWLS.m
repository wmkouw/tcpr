...
sigmate=0.25;
sigmay=0.1;
xtr=mutr+sigmatr*randn(1,ntr);
xte=mute+sigmate*randn(1,nte);
ytr=sinc(xtr)+sigmay*randn(1,ntr);
yte=sinc(xte)+sigmay*randn(1,nte);

xdisp=linspace(-1,3,100);
ydisp=sinc(xdisp);
ptr_xdisp=pdf_Gaussian(xdisp,mutr,sigmatr);
pte_xdisp=pdf_Gaussian(xdisp,mute,sigmate);
w_xdisp=pte_xdisp./ptr_xdisp;

ptr_xtr=pdf_Gaussian(xtr,mutr,sigmatr);
pte_xtr=pdf_Gaussian(xtr,mute,sigmate);
