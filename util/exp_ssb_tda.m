function exp_ssb_tda(Z,yZ,varargin)
...
for r = 1:p.Results.nR
    ...
    for m = 1:lNM
        ...
        % Slice out learning subset
        T = Z(ixnM,:);
        yT = yZ(ixnM);
        if p.Results.setDiff
            V = Z(setdiff(1:M,ixnM),:);
            yV = yZ(setdiff(1:M,ixnM));
        else
            V = Z;
            yV = yZ;
        end
        ...
        switch p.Results.clf
            case 'lda'
                ...
                R{r,m} = sum(sum(ll_lda(theta{r,m}{1}, theta{r,m}{2}, theta{r,m}{3}, V, yV),2),1)./size(V,1);
            ...
            case 'qda'
                ...
                % Evaluate average log-likelihood
                R{r,m} = sum(sum(ll_qda(theta{r,m}{1}, theta{r,m}{2}, theta{r,m}{3}, V, yV),2),1)./size(V,1);
