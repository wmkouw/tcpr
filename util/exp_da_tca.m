...
for r = 1:p.Results.nR
    ...
    for n = 1:lNN
        for m = 1:lNM
            ...
            if isempty(p.Results.lambda)
                ...
                for la = 1:length(Lambda)
                    ...
                    for f = 1:p.Results.nF
                        ...
                        [theta_f,P] = tca(X(ixNN(ixFo~=f),:),yX(ixNN(ixFo~=f)),Z(ixNM,:),'maxIter', p.Results.maxIter, 'xTol', p.Results.xTol, 'nC', p.Results.nC, 'l2', Lambda(la));
                        
                        % Evaluate on held-out source folds (error)
                        B = rbf_kernel(X(ixNN(ixFo==f),:), Z(ixNM,:))*P;
                        [~,pred_f] = max([B ones(length(ixNN(ixFo==f)),1)]*theta_f, [], 1);
                        R_la(la) = mean(pred_f ~= yX(ixNN(ixFo==f)),1);
                    end
                end
                % Select minimal
                R_la(isinf(R_la)) = NaN;
            ...
            [theta{r,n,m},P{r,n,m},R(r,n,m),e(r,n,m),pred{r,n,m},post{r,n,m},AUC(r,n,m)] = tca(X(ixNN,:), yX(ixNN), Z(ixNM,:),'yZ', yZ(ixNM),'maxIter', p.Results.maxIter, 'xTol', p.Results.xTol, 'nC', p.Results.nC, 'l2', lambda);
        end
    end
end
